...
app.get('/', (req, res) => {
    res.status(200).send(tabelacarros);
});
...
app.get('/:sigla', (req, res)  => {
    ...
    res.status(200).send(carro); // Se encontrado a sigla retorna a resposta correta 

});
app.post('/', (req, res) => {
    const novoCarro = req.body; // Obtem o corpo enviado para incluir um carro
    const carroExiste = tabelacarros.find(carro => carro.sigla === novoCarro.sigla);
    if (carroExiste) {
    res.status(400).send('Já existe um carro cadastrado com essa sigla!');
    return;
    } 
    // JoI
